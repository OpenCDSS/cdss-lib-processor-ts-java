...
/**
Add the date to the list of Np missing dates that are ignored.
These should be few and far between, occurring only when the NWS does an ESP run that
ends a day short (e.g., March 30 instead of March 31.  Rather than throw away the whole
month bin, the number of points considered in the month is reduced by the allowed
number (generally 1).
These can be used to print footnotes on reports.
*/
private void addDateAsIgnoredNpMissing ( Vector ignored_Np_missing_dates, DateTime date )
{
    // Need to search what is already added because the same dates are processed for
    // some weekly and monthly bins.
    int size = ignored_Np_missing_dates.size();
    boolean found = false;
    DateTime d;
    for ( int i = 0; i < size; i++ ) {
        d = (DateTime)ignored_Np_missing_dates.elementAt(i);
        if ( d.equals(date) ) {
            found = true;
            break;
        }
    ...
    if ( !found ) {
        ignored_Np_missing_dates.addElement ( date );
    }
}
...
private void createAdvanceAnalysisReport ( String title, String AdvanceAnalysisOutputFile, DateTime STPDate_DateTime,
        DateTime report_start_DateTime,
        TS Np, TS Nc, TS R,
        double [] Np_mean, double [] Nc_mean, double [] R_mean, DateTime [] bin_end_DateTime,
        List ignored_holiday_dates, List ignored_Np_missing_dates, int nNp_missing_atendofmonth_allowed )
throws Exception
{
    //String comment = "#";
    CommandProcessor processor = getCommandProcessor();
    ...
    */
    ...
    DateTime date = new DateTime(report_start_DateTime);
    double Np_value;
    double R_value;
    double Nc_value;
    ...
    printIgnoredHolidaysFootnote ( fout, ignored_holiday_dates, delim3 );
    // Print footnote if any missing data at the end of NP
    printIgnoredNpMissingFootnote ( fout, ignored_Np_missing_dates, delim3, nNp_missing_atendofmonth_allowed );
    fout.println ( "" + delim3 );
...
{   String routine = "CustomCommand.customCommand";
    ...
    // Allowed missing values at the end of a monthly bin (set to 1 to account for
    // one day sometimes being left off (leap year?)...
    int nNp_missing_atendofmonth_allowed = 1;
    
    // Dates that are ignored because Np is missing at the end of a month...
    
    Vector ignored_Np_missing_dates = new Vector();
    ...
    DateTime start_date = new DateTime(STPDate_DateTime);
    if ( start_date.getWeekDay() == 0 ) {
    ...
    else if ( start_date.getWeekDay() == 1 ) {
    ...
    else if ( start_date.getWeekDay() != 2 ) {
    ...
    if ( (TimeUtil.numDaysInMonth(Np_end_DateTime.getMonth(),Np_end_DateTime.getYear()) - Np_end_DateTime.getDay())
        ...
        message = "The Np end date/time \"" + Np_end_DateTime + "\" is not within " +
            nNp_missing_atendofmonth_allowed + ".  The entire month will be considered as NO DATA.";
        Message.printWarning( 3, routine, message );
        status.addToLog ( CommandPhaseType.RUN,
                new CommandLogRecord(CommandStatusType.WARNING,
                        message, "Allowing but first bin results will have fewer values averaged." ) );
    ...
    while ( start_date.getWeekDay() <= 2 ) {
        start_date.addDay ( 1 );
        // Will break when day is 3 (Wednesday) or higher
    }
    DateTime end = null;
    if ( Nc_TS != null ) {
    ...
    Message.printStatus ( 2, routine, "Generating report for period " + start_date + " through " + end );
    ...
    for ( int i = 0; date.lessThanOrEqualTo(end); date.addDay(1), i++ ) {
        ...
        if ( R_TS.isDataMissing(R_value) && date.lessThanOrEqualTo(R_end_DateTime)) {
            // Should not have missing within the period of the R
        ...
        if ( (Nc_TS != null) && Nc_TS.isDataMissing(Nc_value) && date.lessThanOrEqualTo(Nc_end_DateTime)) {
            // Should not have missing within the period of Nc
        ...
        if ( Np_TS.isDataMissing(Np_value) && date.lessThanOrEqualTo(Np_end_DateTime) ) {
            // Should not have missing within the period of Nc
        ...
        else if ( Np_value < 0.0 && date.lessThanOrEqualTo(Np_end_DateTime) ) {
        ...
        // Determine the bin that the value should go into, starting with figuring out the week.
        // A weekly and monthly bin may be active at the same time.
        if ( i == 0 ) {
        ...
        int bin_month = -1;
        if ( isHoliday(date) ) {
            // Just skip it as if it did not exist - don't add to missing or non-missing
        ...
        if ( bin_month >= 0 ) {
            ...
            // Adjust the data for special considerations.  If Np is missing an the date is within the allowed
            // number of days of missing from the end of the month, exclude Nc and R.
            // Otherwise, Nc and R will be added and the check later will determine that different number
            // of days are available and the test will pass.
            if ( Np_is_missing && dateAtEndOfNpLastMonth ( Np_end_DateTime, date, nNp_missing_atendofmonth_allowed ) ) {
                Message.printStatus( 2, routine,
                        "Not adding " + date + " to a month bin because Np is missing value at end of month." );
                addDateAsIgnoredNpMissing ( ignored_Np_missing_dates, new DateTime(date) );
                // Skip similar to a holiday
            }
            else {
    ...
    try {
         ...
         // Print footnote if any missing data at the end of NP
         printIgnoredNpMissingFootnote ( fout, ignored_Np_missing_dates, delim6, nNp_missing_atendofmonth_allowed );
         fout.println ( delim6 );
         ...
         createAdvanceAnalysisReport ( Title, AdvanceAnalysisOutputFile_full, STPDate_DateTime, start_date,
                 Np_TS, Nc_TS, R_TS, Np_mean, Nc_mean, R_mean, bin_end_DateTime,
                 ignored_holiday_dates, ignored_Np_missing_dates, nNp_missing_atendofmonth_allowed );
...
/**
Determine whether the given date is at the end of the Np data and within the allowed
number of missing days.
*/
private boolean dateAtEndOfNpLastMonth ( DateTime Np_end_DateTime, DateTime date, int nNp_missing_atendofmonth_allowed )
{   String routine = "CustomeCommand_Command.dateAtEndOfNpLastMonth";
    // Initialize to the end of month for the time series...
    DateTime eom = new DateTime ( Np_end_DateTime );
    eom.setDay( TimeUtil.numDaysInMonth(eom));
    DateTime allowed_start = new DateTime ( eom );
    // Decrement by the number of allowed days from end...
    // For example if 1 day of missing is allowed and date is March 30, then
    // the end of the month is March 31 and only that day would be allowed to be missing.
    allowed_start.addDay( 1 -nNp_missing_atendofmonth_allowed );
    // Allowed Np end is the number of days prior to the end of the month
    DateTime Np_end_allowed = new DateTime ( eom );
    Np_end_allowed.addDay ( -nNp_missing_atendofmonth_allowed );
    // To be in allowed range, the date and the Np end must be in the allowed range.
    if ( date.greaterThanOrEqualTo(allowed_start) && date.lessThanOrEqualTo(eom) &&
            Np_end_DateTime.greaterThanOrEqualTo(Np_end_allowed) && date.lessThanOrEqualTo(eom) ) {
        Message.printStatus( 2, routine,
                "Allowed start="+allowed_start +
                " eom=" + eom +
                " date=" + date + "Npend=" + Np_end_DateTime + " Missing data in Np on " + date +
                " is in end allowed period (ignore data).");
        return true;
    }
    else {
        Message.printStatus( 2, routine,
                ...
                " eom=" + eom +
                " date=" + date + "Npend=" + Np_end_DateTime + "Missing data in Np on " + date +
                " is not in end allowed period (treat as missing).");
        return false;
    }
}
...
/**
Print the list of days with Np missing that were ignored in processing the data.
@param fout PrintWriter for output.
@param ignored_holiday_dates List of holiday dates that were ignored.
@param line_end Text to put at end of line (e.g., list of delimiters for blank fields).
*/
private void printIgnoredNpMissingFootnote ( PrintWriter fout, List ignored_Np_missing_dates, String line_end,
        int nNp_missing_atendofmonth_allowed )
{
    int size = ignored_Np_missing_dates.size();
    if ( size > 0 ) {
        // Blank line...
    ...
    DateTime date = null;
    for ( int i = 0; i < size; i++ ) {
        date = (DateTime)ignored_Np_missing_dates.get(i);
        fout.println (
                "Ignored day in bin averages because Np missing at end of month on date: " + date + line_end );
    }
}
...
{   String routine = "CustomCommand_test";
    ...
    if ( R_missing > 0 || Nc_missing > 0 || (R_notmissing == 0) || (Nc_notmissing == 0) ) {
        ...
        Message.printStatus ( 2, routine, "Value test different number of R, Nc nonmissing for bin " + __BIN_NAMES[bin] );
    }
