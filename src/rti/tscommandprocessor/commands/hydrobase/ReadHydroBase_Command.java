...
which is equal to the HydroBase SPFlex maximum minus (data type and interval).
*/
...
throws InvalidCommandParameterException, CommandWarningException, CommandException {
	...
	*/
	...
	// TODO SAM 2006-04-27 Code cleanup As per Ray Bennett always do this.
	...
	try {
        ...
        if ( (TSID != null) && !TSID.equals("") ) {
			...
			if ( HydroBase_Util.isStationTimeSeriesDataType ( hbdmi, hbMeasType ) ){
				...
				// Structures (no SFUT).
				is_Structure = true;
				filterPanel = new HydroBase_GUI_StructureGeolocStructMeasType_InputFilter_JPanel ( hbDataStore, false, 6, -1 );
			...
			List<HydroBase_AgriculturalCASSCropStats> cassCropStatsTSCatalogList = new ArrayList<>();
			List<HydroBase_AgriculturalCASSLivestockStats> cassLivestockStatsTSCatalogList = new ArrayList<>();
			List<HydroBase_StationGeolocMeasType> stationTSCatalogList = new ArrayList<>();
			List<HydroBase_StructureGeolocStructMeasType> structureTSCatalogList = new ArrayList<>();
			List<HydroBase_AgriculturalNASSCropStats> nassCropStatsTSCatalogList = new ArrayList<>();
			List<HydroBase_CUPopulation> cupopTSCatalogList = new ArrayList<>();
			List<HydroBase_StructureIrrigSummaryTS> irrigSummaryTSCatalogList = new ArrayList<>();
			List<HydroBase_WISSheetNameWISFormat> wisTSCatalogList = new ArrayList<>();
			List<HydroBase_GroundWaterWellsView> groundwaterWellsTSCatalogList = new ArrayList<>();
			...
			String inputName = "";
			// Input name to add to identifiers, if it has been requested (this includes the tilde character).
			if ( (DataStore != null) && !DataStore.equals("") ) {
			    // Use the datastore for the time series input name
				// (will not include "HydroBase" since datastore has the connection information).
			    inputName = "~" + DataStore;
			}
			else {
			    // Use the input name.
...
public String toString ( PropList parameters, int majorVersion ) {
	...
	// Only add where parameters that are confirmed.
	String delim = ";";
