// MergeTableColumns_JDialog - editor for MergeTableColumns command
...
public class MergeTableColumns_JDialog extends JDialog
		...
		if ( !__error_wait ) {
			if ( __runnable ) {
				...
				try {	commitEdits ();
					...
					__command.getCommandName() + "_JDialog",
					"There was an error running the " +
					"command.");
					Message.printWarning ( 3, routine, e );
				}
			}
			else {	// Command has been edited...
				response ( true );
			}
		}
	}
...
{	__parent_JFrame = parent;
	...
	"This " + name+" merges columns in a list file, creating a new column" +
	" in a new list file."),
	...
	if ( runnable ) {
		...
		__cancel_JButton.setToolTipText (
		"Close window without running tool." );
		button_JPanel.add (__cancel_JButton);
		__ok_JButton = new SimpleJButton("Run Tool", this);
		button_JPanel.add (__ok_JButton);
		__ok_JButton.setToolTipText (
		"Run tool and keep window open." );
	}
	else {	__cancel_JButton = new SimpleJButton("Cancel", this);
		button_JPanel.add (__cancel_JButton);
		__cancel_JButton.setToolTipText (
		...
		button_JPanel.add (__ok_JButton);
		__ok_JButton.setToolTipText (
		"Close window and save changes to command." );
	}
