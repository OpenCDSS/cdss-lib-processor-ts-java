// ReadTableFromXML_JDialog - editor dialog for ReadTableFromXML command
...
public class ReadTableFromXML_JDialog extends JDialog
...
private JTextField __RowElement_JTextField = null;
...
private ReadTableFromXML_Command __command = null;
...
*/
public ReadTableFromXML_JDialog ( JFrame parent, ReadTableFromXML_Command command )
{	super(parent, true);
	...
	if ( o == __browse_JButton ) {
		...
		if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				Message.printWarning ( 1,"ReadTableFromXML_JDialog", "Error converting file to relative path." );
			}
		...
		HelpViewer.getInstance().showHelp("command", "ReadTableFromXML");
	}
...
{	// Put together a list of parameters to check...
	...
	String RowElement = __RowElement_JTextField.getText().trim();
    ...
    if ( RowElement.length() > 0 ) {
        props.set ( "RowElement", RowElement );
    }
    if ( Top.length() > 0 ) {
        props.set ( "Top", Top );
    }
...
{	String TableID = __TableID_JTextField.getText().trim();
	...
	__command.setCommandParameter ( "RowElement", RowElement );
...
*/
private void initialize ( JFrame parent, ReadTableFromXML_Command command )
{	__command = command;
	...
	int yy = -1;
   	...
   	JGUIUtil.addComponent(paragraph, new JLabel (
   		"This command reads a table from an XML file.  The table can then be used by other commands."),
		0, ++yy, 8, 1, 0, 0, insetsTLBR, GridBagConstraints.BOTH, GridBagConstraints.WEST);
   	JGUIUtil.addComponent(paragraph, new JLabel (
   		"Use the WebGet() command to retrieve files from the web."),
		0, ++yy, 8, 1, 0, 0, insetsTLBR, GridBagConstraints.BOTH, GridBagConstraints.WEST);
    JGUIUtil.addComponent(paragraph, new JLabel (
		"The XML elements to read must be able to be \"flattened\" into table rows."),
		0, ++yy, 8, 1, 0, 0, insetsTLBR, GridBagConstraints.BOTH, GridBagConstraints.WEST);	
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel ("Row element:"),
        0, ++y, 1, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.EAST);
    __RowElement_JTextField = new JTextField (30);
    __RowElement_JTextField.addKeyListener (this);
    JGUIUtil.addComponent(main_JPanel, __RowElement_JTextField,
        1, y, 2, 1, 1, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
    JGUIUtil.addComponent(main_JPanel, new JLabel ("Required - element name corresponding to rows (default=child of root)."),
...
{	String TableID = "";
	...
	if (__first_time) {
		...
		RowElement = props.getValue ( "RowElement" );
        ...
        if ( RowElement != null ) {
            __RowElement_JTextField.setText ( RowElement );
        }
        if ( Top != null ) {
	...
	RowElement = __RowElement_JTextField.getText().trim();
	Top = __Top_JTextField.getText().trim();
	props = new PropList ( __command.getCommandName() );
    props.add ( "TableID=" + TableID );
	props.add ( "InputFile=" + InputFile );
	props.add ( "RowElement=" + RowElement );
	props.add ( "Top=" + Top );
