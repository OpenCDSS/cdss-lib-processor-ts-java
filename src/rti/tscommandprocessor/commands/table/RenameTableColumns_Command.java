// RenameTableColumns_Command - this class initializes, checks, and runs the RenameTableColumns() command.
...
*/
public class RenameTableColumns_Command extends AbstractCommand implements Command
{

/**
Constructor.
*/
public RenameTableColumns_Command ()
{	super();
	setCommandName ( "RenameTableColumns" );
}
...
{	String TableID = parameters.getValue ( "TableID" );
    ...
    if ( (ColumnMap == null) || (ColumnMap.length() == 0) ) {
        message = "The column map must be specified.";
        warning += "\n" + message;
        status.addToLog ( CommandPhaseType.INITIALIZATION,
            new CommandLogRecord(CommandStatusType.FAILURE,
                message, "Specify the column map." ) );
    }
...
{	List<String> tableIDChoices =
	...
	return (new RenameTableColumns_JDialog ( parent, this, tableIDChoices )).ok();
}
...
{	String routine = getClass().getSimpleName() + ".runCommandInteral",message = "";
	...
	try {
    	// Join the tables...
	    if ( commandPhase == CommandPhaseType.RUN ) {
	        table.renameFields ( table, columnMap, problems );
		...
		message = "Unexpected error renaming columns (" + e + ").";
