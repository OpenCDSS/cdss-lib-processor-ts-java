...
throws InvalidCommandParameterException, CommandWarningException, CommandException {
    ...
    */
    ...
    try {
    	// If prepend and/or append files were specified, read and use for the text:
    	// - append to the initial text
    	StringBuilder prependText = new StringBuilder();
    	if ( PrependText != null ) {
    		prependText.append ( PrependText );
    	}
    	if ( (PrependFile != null) && !PrependFile.isEmpty() ) {
    		String prependFile_full = IOUtil.verifyPathForOS(
             	IOUtil.toAbsolutePath(TSCommandProcessorUtil.getWorkingDir(processor),
              		TSCommandProcessorUtil.expandParameterValue(processor, this,PrependFile)) );
    		File prependFile = new File ( prependFile_full );
    		if ( !prependFile.exists() ) {
    			message = "The prepend file \"" + prependFile_full + "\" does not exist.";
    			Message.printWarning ( warning_level,
    				MessageUtil.formatMessageTag( command_tag,++warning_count), routine, message );
    			status.addToLog ( CommandPhaseType.RUN, new CommandLogRecord(CommandStatusType.WARNING,
    				message, "Verify that the file exixts." ) );
    		}
    		else {
    			// Expand the file and append to the text.
    			prependText.append(
              		TSCommandProcessorUtil.expandParameterValue(processor, this,
              			IOUtil.fileToStringBuilder(prependFile_full).toString()));
    		}
    	}
    	...
    	if ( (AppendFile != null) && !AppendFile.isEmpty() ) {
    		String appendFile_full = IOUtil.verifyPathForOS(
             	IOUtil.toAbsolutePath(TSCommandProcessorUtil.getWorkingDir(processor),
              		TSCommandProcessorUtil.expandParameterValue(processor, this,AppendFile)) );
    		File appendFile = new File ( appendFile_full );
    		if ( !appendFile.exists() ) {
    			message = "The append file \"" + appendFile_full + "\" does not exist.";
    			Message.printWarning ( warning_level,
    				MessageUtil.formatMessageTag( command_tag,++warning_count), routine, message );
    			status.addToLog ( CommandPhaseType.RUN, new CommandLogRecord(CommandStatusType.WARNING,
    				message, "Verify that the file exixts." ) );
    			...
    			appendText.append(
              		TSCommandProcessorUtil.expandParameterValue(processor, this,
    					IOUtil.fileToStringBuilder(appendFile_full).toString()));
    		}
    	}
...
@param prependText text to prepend to the markdown file
@param appendText text to append to the markdown file
...
private int writeMarkdown ( DataTable table,
	...
	StringBuilder prependText, StringBuilder appendText,
...
throws Exception {
	...
	int irow = 0, icol = 0;
	try {
		// If prepend text is specified, add before the table:
		// - this may be a header and other content to explain the table
		// - no ending newline is written (if needed, supply in the insert text)
		if ( prependText.length() > 0 ) {
			writer.write(prependText.toString());
		}

    	// If any comments have been passed in, print them at the top of the file:
		// - typically these are not output, especially if using prepend content
		...
		// If append text is specified, add after the table:
		// - this may be a footer and other content to explain the table
