...
{	String TSList = parameters.getValue ( "TSList" );
    ...
    */
    ...
    valid_Vector.add ( "AdjustMethod" );
    valid_Vector.add ( "ExtremeToAdjust" );
    valid_Vector.add ( "ExtremeValue" );
...
{	int warning_level = 2;
	...
	if ( (command_string.indexOf('=') > 0) || command_string.endsWith("()") ) {
		...
		String AdjustMethod = ((String)v.elementAt(2)).trim();
		String ExtremeToAdjust = ((String)v.elementAt(3)).trim();
		String ExtremeValue = ((String)v.elementAt(4)).trim();
		String MaxIntervals = ((String)v.elementAt(5)).trim();
		String AnalysisStart = ((String)v.elementAt(6)).trim();
		String AnalysisEnd = ((String)v.elementAt(7)).trim();
...
{	String routine = "AdjustExtremes_Command.runCommand", message;
	int warning_count = 0;
	int warning_level = 2;
    ...
    /*
	PropList props = new PropList ( "AdjustExtremes" );
	if ( FillFlag != null ) {
	...
	for ( int its = 0; its < nts; its++ ) {
		...
		try {
		    TSUtil.adjustExtremes ( ts, AdjustMethod, ExtremeToAdjust, ExtremeValue_double,
	                MaxIntervals_int, AnalysisStart_DateTime, AnalysisEnd_DateTime );
		}
		catch ( Exception e ) {
