...
private FillPattern_Command __command = null;
...
private SimpleJComboBox	__PatternID_JComboBox = null;
...
*/
public FillPattern_JDialog ( JFrame parent, FillPattern_Command command )
{	super(parent, true);
...
{	Object o = event.getSource();
	...
	if ( o == __cancel_JButton ) {
		...
		HelpViewer.getInstance().showHelp("command", "FillPattern");
	}
...
{   // Put together a list of parameters to check...
    ...
    String PatternID = __PatternID_JComboBox.getSelected();
...
@param command Command to edit.
*/
private void initialize ( JFrame parent, FillPattern_Command command )
{	__command = command;
    String routine = getClass().getSimpleName() + ".initialize";
	
    // Get the patterns
    List<TS> patternTS_List = null;
    try {
        patternTS_List = TSCommandProcessorUtil.getPatternTSListFromCommandsBeforeCommand(
                (TSCommandProcessor)__command.getCommandProcessor(), __command );
        __command.getCommandProcessor().getPropContents("PatternTSList");
    }
    catch ( Exception e ) {
        Message.printWarning ( 3, routine, e );
        Message.printWarning ( 1, routine,
            "You must define pattern files with ReadPatternFile() before " +
            "inserting a FillPattern() command." );
            response ( false );
    }
    List<String> PatternID_List = new Vector<String>();
    if ( patternTS_List != null ) {
        for ( int i = 0; i < patternTS_List.size(); i++ ) {
            TS ts = patternTS_List.get(i);
            PatternID_List.add ( ts.getLocation() );
        }
    }
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel (
		"Monthly time series can be filled using historical average patterns (e.g., WET, DRY, AVG climate patterns)." ), 
		0, ++y, 7, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
    JGUIUtil.addComponent(main_JPanel, new JLabel (
        "Patterns are read using ReadPatternFile() command(s).  The AnalyzePattern() command creates the patterns."),
        0, ++y, 7, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
	...
	int size = PatternID_List.size();
	if ( size == 0 ) {
		Message.printWarning ( 1, "FillPattern_JDialog.initialize",
		"You must define pattern files with ReadPatternFile()\nbefore " +
		"inserting a FillPattern() command." );
		response ( false );
	}
	__PatternID_JComboBox.setData ( PatternID_List );
...
{   String routine = "FillPattern_JDialog.refresh";
    ...
    if ( __first_time ) {
       ...
       if ( EnsembleID == null ) {
        ...
        if ( PatternID == null ) {
            // Select default...
            __PatternID_JComboBox.select ( 0 );
        }
        else {
            if ( JGUIUtil.isSimpleJComboBoxItem( __PatternID_JComboBox,PatternID, JGUIUtil.NONE, null, null ) ) {
