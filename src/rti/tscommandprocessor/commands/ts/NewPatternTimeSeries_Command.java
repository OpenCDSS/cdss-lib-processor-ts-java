// NewPatternTimeSeries_Command - This class initializes, checks, and runs the NewPatternTimeSeries() command.
	...
	if ( (PatternValues != null) && !PatternValues.isEmpty() ) {
		...
		for ( int i = 0; i < size; i++ ) {
			...
			if ( token.length() == 0 ) {
                ...
                status.addToLog(CommandPhaseType.INITIALIZATION,
					new CommandLogRecord(
					CommandStatusType.FAILURE, message, "Provide a valid number."));
			}
			else {
