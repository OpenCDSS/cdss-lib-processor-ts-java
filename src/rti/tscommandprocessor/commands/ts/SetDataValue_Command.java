// SetDataValue_Command - This class initializes, checks, and runs the SetDataValue() command.
...
{	int warning_level = 2;
	...
	if ( (command_string.indexOf('=') > 0) || command_string.endsWith("()") ) {
		...
		// TODO SAM 2008-09-12 This whole block of code needs to be
...
{	String routine = "SetDataValue_Command.runCommand", message;
	...
	if ( SetDateTime != null ) {
		...
		request_params.set ( "DateTime", SetDateTime );
		bean = null;
		try {
            bean = processor.processRequest( "DateTime", request_params);
		}
		catch ( Exception e ) {
			message = "Error requesting SetDateTime DateTime(DateTime=" + SetDateTime + ") from processor.";
		...
		if ( prop_contents == null ) {
			message = "Null value for SetDateTime DateTime(DateTime=" + SetDateTime + "\") returned from processor.";
			Message.printWarning(log_level,
				MessageUtil.formatMessageTag( command_tag, ++warning_count),
				routine, message );
            status.addToLog ( CommandPhaseType.RUN,
		...
		message = "SetDateTime \"" + SetDateTime + "\" is invalid.";
		Message.printWarning(warning_level,
				MessageUtil.formatMessageTag( command_tag, ++warning_count),
				routine, message );
        status.addToLog ( CommandPhaseType.RUN,
    ...
    */
	...
	for ( int its = 0; its < nts; its++ ) {
		...
		try {
	        ts.setDataValue ( SetDateTime_DateTime, NewValue_double );
	        ts.addToGenesis ( "Set " + NewValue + " on " + SetDateTime );
		}
		catch ( Exception e ) {
			message = "Unexpected error setting time series \"" + ts.getIdentifier() + "\" value (" + e + ").";
            Message.printWarning ( warning_level,
...
{	if ( props == null ) {
	...
	String SetDateTime = props.getValue( "SetDateTime" );
	...
	if ( (SetDateTime != null) && (SetDateTime.length() > 0) ) {
		...
		b.append ( "SetDateTime=\"" + SetDateTime + "\"" );
	}
