// SetIgnoreLEZero_Command - This class initializes, checks, and runs the SetIgnoreLEZero() command.
...
{	String routine = "SetIgnoreLEZero_Command.runCommand", message;
	...
	try {
		...
		if ( IgnoreLEZero.equalsIgnoreCase(_True) ) {
		    ...
		    processor.setPropContents ( "IgnoreLEZero", Boolean.valueOf(true) );
		}
		else if ( IgnoreLEZero.equalsIgnoreCase(_False) ){
		    Message.printStatus ( 2, routine,
            "Values <= 0 WILL NOT be treated as missing when computing historic averages." );
		    processor.setPropContents ( "IgnoreLEZero", Boolean.valueOf(false) );
		}
	}
	catch ( Exception e ) {
