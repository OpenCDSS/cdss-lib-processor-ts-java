// TSID_JDialog - Editor dialog for time series identifiers.
...
// TODO SAM 2009-01-15 Implement field-based time series editor,
// consistent with that used in other commands where NewTSID is entered, like the NewTimeSeries() command.
/**
Editor dialog for time series identifiers.
Currently a simple text field is provided but in the future separate fields may be used for TSID parts.
*/
	...
	if ( o == __cancel_JButton ) {
        ...
        if ( __path_JButton.getText().equals( __AddWorkingDirectory) ) {
            ...
            try {
                ...
                Message.printWarning ( 1, "TSID_JDialog", "Error converting file to relative path." );
            }
        }
        refresh ();
    }
...
/**
Indicate whether the time series identifier has a filename.
This is determined from the input name (from ~InputType~InputName).
...
private void initialize ( JFrame parent, TSID_Command command ) {
    ...
    // FIXME SAM 2009-01-15 Figure out how to not hard code here.
    // Only want to show the path button for input types that are for files (not databases).
