...
throws InvalidCommandParameterException, CommandWarningException, CommandException {
	...
	// If the Program has been specified but CommandLine has not, construct the command line from the program for logging.
	// The Program will take precedence below so this will not change the logic.
	...
	try {
        ...
        //props = null;
        // pmg = null;
        // Normally can do this, although TSTool may sit for awhile until the process is finished
		// (need to figure out a way to make TSTool wait on the thread without hanging).
        ProcessManager pm = null;
        if ( (Program != null) && !Program.isEmpty() ) {
            // Specify the program to run using individual strings - this takes precedence over the full command line.
        ...
        if ( outputCheckTable != null ) {
        	...
        	if ( (fileColumnNum >= 0) && (patternColumnNum >= 0) && (levelColumnNum >= 0) && (messageColumnNum >= 0) && (recommendationColumnNum >= 0) ) {
	        	...
	        	for ( int iFile = 0; iFile < outputFiles.size(); iFile++ ) {
	        		...
	        		patternList[iFile] = new ArrayList<>();
	        		levelList[iFile] = new ArrayList<>();
	        		messageList[iFile] = new ArrayList<>();
	        		recommendationList[iFile] = new ArrayList<>();
	        		for ( int irow = 0; irow < outputCheckTable.getNumberOfRecords(); irow++ ) {
		        		TableRecord rec = outputCheckTable.getRecord(irow);
		        		String file = rec.getFieldValueString(fileColumnNum);
...
@param file the stderr file for the program being run
*/
...
@param file the stdout file for the program being run
*/
