// WriteWaterML2_JDialog - Command editor dialog for the WriteWaterML2() command.
...
/**
Command editor dialog for the WriteWaterML2() command.
*/
@SuppressWarnings("serial")
public class WriteWaterML2_JDialog extends JDialog
...
private WriteWaterML2_Command __command = null;
...
private SimpleJComboBox __Dialect_JComboBox = null;
...
/**
WriteWaterML_JDialog constructor.
@param parent JFrame class instantiating this class.
@param command Command to edit.
*/
public WriteWaterML2_JDialog ( JFrame parent, WriteWaterML2_Command command )
{	super(parent, true);
...
{	Object o = event.getSource();
	...
	if ( o == __browse_JButton ) {
		...
		sff = new SimpleFileFilter("xml", "WaterML 2 Time Series File");
		fc.addChoosableFileFilter(sff);
		sff = new SimpleFileFilter("waterml", "WaterML 2 Time Series File");
		fc.addChoosableFileFilter(sff);
		sff = new SimpleFileFilter("wml", "WaterML 2 Time Series File");
		fc.addChoosableFileFilter(sff);
		...
		if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
			...
			if (path != null) {
				...
				try {
					...
					Message.printWarning ( 1,"WriteWaterML2_JDialog", "Error converting file to relative path." );
				}
		...
		HelpViewer.getInstance().showHelp("command", "WriteWaterML2");
	}
		...
		if ( __path_JButton.getText().equals(__AddWorkingDirectory) ) {
			...
			try {
				...
				Message.printWarning ( 1, "WriteWaterML_JDialog", "Error converting file to relative path." );
			}
		}
		refresh ();
	}
...
{	// Put together a list of parameters to check...
	...
	String Dialect = __Dialect_JComboBox.getSelected();
    ...
    if (Dialect.length() > 0) {
        parameters.set("Dialect", Dialect);
    }
...
{	String TSList = __TSList_JComboBox.getSelected();
	...
	__command.setCommandParameter ( "Dialect", Dialect );
...
@param command Command to edit.
*/
private void initialize ( JFrame parent, WriteWaterML2_Command command )
{	__command = command;
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel (
        "<html><b>This command is under development to evaluate writing WaterML 2.0 file format in a standard way.</b></html>" ),
        0, ++y, 8, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
    JGUIUtil.addComponent(main_JPanel, new JLabel (
		"Write time series to a WaterML 2 format file," +
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel (
        "Enter date/times to a precision appropriate for time series."),
		0, ++y, 8, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel ( "WaterML 2 file to write:" ), 
    ...
    __WriteMethod_JComboBox.setToolTipText(__command._API + " will use objects generated from WaterML 2 XML schema, " +
    	__command._CreateDOM + " will create XML document directly.");
    __WriteMethod_JComboBox.add("");
    __WriteMethod_JComboBox.add(__command._API);
    __WriteMethod_JComboBox.add(__command._CreateDOM);
    // Select a default...
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel ( "Optional - how to write (default=" + __command._CreateDOM + ")."),
        3, y, 2, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
        
    JGUIUtil.addComponent(main_JPanel, new JLabel ("WaterML 2 version:"),
        0, ++y, 1, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.EAST);
    ...
    JGUIUtil.addComponent(main_JPanel, __Version_JComboBox,
        1, y, 2, 1, 1, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
        JGUIUtil.addComponent(main_JPanel, new JLabel ("Optional - WaterML version (currently only 2.0 is supported)."),
        3, y, 3, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST );
        
    JGUIUtil.addComponent(main_JPanel, new JLabel ("WaterML 2 dialect:"),
        0, ++y, 1, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.EAST);
    __Dialect_JComboBox = new SimpleJComboBox(false);
    __Dialect_JComboBox.add ( "" ); // Most recent
    __Dialect_JComboBox.add ( WaterML2Writer.USGS );
    __Dialect_JComboBox.select ( 0 );
    __Dialect_JComboBox.addActionListener (this);
    JGUIUtil.addComponent(main_JPanel, __Dialect_JComboBox,
        1, y, 2, 1, 1, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
        JGUIUtil.addComponent(main_JPanel, new JLabel ("Optional - WaterML dialect (default=" + WaterML2Writer.USGS + ")."),
        3, y, 3, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST );
...
{	String routine = getClass().getSimpleName() + ".refresh";
	...
	if ( __first_time ) {
	    ...
	    Dialect = parameters.getValue("Dialect");
        ...
        if ( WriteMethod == null || WriteMethod.equals("") ) {
            ...
            if ( JGUIUtil.isSimpleJComboBoxItem( __WriteMethod_JComboBox, WriteMethod, JGUIUtil.NONE, null, null ) ) {
                ...
                Message.printWarning ( 1, routine, "Existing command references an invalid WriteMethod \"" +
                	WriteMethod + "\".  Select a different choice or Cancel." );
            }
        }
        if ( Version == null ) {
            // Select default...
        ...
        if ( Dialect == null ) {
            ...
            if ( JGUIUtil.isSimpleJComboBoxItem( __Dialect_JComboBox, Dialect, JGUIUtil.NONE, null, null ) ) {
                __Dialect_JComboBox.select ( Dialect );
            }
            else {
                Message.printWarning ( 1, routine, "Existing command references an invalid\nDialect value \"" +
                    Dialect + "\".  Select a different value or Cancel.");
                __error_wait = true;
            }
        }
        if (Precision != null) {
	...
	Dialect = __Dialect_JComboBox.getSelected();
	...
	parameters.add ( "Dialect=" + Dialect );
